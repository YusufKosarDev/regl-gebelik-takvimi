...
/**
 * Every `ALTER TABLE` in a run adds a column, and does nothing else.
 *
 * These tests used to ban `ALTER TABLE` outright, which said what was meant
 * while every migration happened to be a `CREATE`. It is not the rule: the rule
 * is that an upgrade must not rewrite or discard what somebody already
 * recorded, and adding a column with a default does neither. A rename, a drop
 * or a rebuild still would, and each of those is an `ALTER` this refuses.
 */
function expectOnlyAdditiveAlters(sql: string): void {
  for (const [statement] of sql.matchAll(/ALTER TABLE [^;]*/gi)) {
    expect(statement).toMatch(/ALTER TABLE \w+ ADD COLUMN/i);
  }
}
...
describe('runMigrations when the database is newer than this build', () => {
  ...
  it('names both the found and the supported version', async () => {
    const spy = createDatabaseSpy({ user_version: 10 });
...
describe('runMigrations applying version 1 to an empty database', () => {
  ...
  it('adds none of the columns we deliberately left out', async () => {
    ...
    // The tables that hold health data carry no bookkeeping of their own: no
    // timestamps, no owner, no sync flags, no tombstones. `sync_state` is a
    // different kind of table — it is bookkeeping — and is left out of this.
    const healthTables = statementsFrom(spy)
      .join(' ')
      .toLowerCase()
      .split('create table sync_state')[0];
    ...
    for (const column of [
      ...
      expect(healthTables).not.toContain(column);
    }
  });
  ...
  it('gives each version its own transaction', async () => {
    ...
    // committed.
    expect(spy.withTransactionAsync).toHaveBeenCalledTimes(8);
  });
...
describe('runMigrations applying version 2 to an empty database', () => {
  ...
  it('passes through version 2 on the way to the latest', async () => {
    ...
    expect(statements[statements.length - 1]).toBe('PRAGMA user_version = 8');
  });
});
...
describe('runMigrations upgrading a version 2 database', () => {
  ...
  it('leaves the cycle tables alone', async () => {
    ...
    // Nothing drops, deletes from or rewrites what is already stored.
    expect(sql).not.toMatch(/DROP TABLE/i);
    expect(sql).not.toMatch(/DELETE FROM/i);
    expect(sql).not.toMatch(/UPDATE (cycle_settings|period_records)/i);
    expectOnlyAdditiveAlters(sql);
  });
...
describe('runMigrations applying version 4 to an empty database', () => {
  ...
  it('passes through version 4 on the way to the latest', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });
  ...
  it('runs the eight versions in order', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });
...
describe('runMigrations upgrading a version 2 database to the latest', () => {
  it('runs the remaining steps in order', async () => {
    const spy = createDatabaseSpy({ user_version: 2 });
...
describe('runMigrations applying version 5 to an empty database', () => {
  it('creates the reminder table', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);

    expect(statementsFrom(spy).join(' ')).toMatch(/CREATE TABLE notification_preferences/i);
  });
  ...
  it('pins it to a single row', async () => {
    ...
    expect(normalize(statementsFrom(spy).join(' '))).toMatch(
      /CREATE TABLE notification_preferences \( id INTEGER PRIMARY KEY CHECK \(id = 1\)/i
    );
  });

  it('constrains each switch to 0 or 1', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = normalize(statementsFrom(spy).join(' '));

    // SQLite has no boolean, and an unconstrained column would happily store a
    // 2 that nothing above it could read.
    for (const column of ['period_reminder_enabled', 'pregnancy_weekly_reminder_enabled']) {
      expect(sql).toMatch(new RegExp(`${column} INTEGER NOT NULL DEFAULT 0`, 'i'));
      expect(sql).toMatch(new RegExp(`CHECK \\(${column} IN \\(0, 1\\)\\)`, 'i'));
    }
  });

  it('defaults both reminders to off', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = normalize(statementsFrom(spy).join(' '));

    expect(sql).toMatch(/period_reminder_enabled INTEGER NOT NULL DEFAULT 0/i);
    expect(sql).toMatch(/pregnancy_weekly_reminder_enabled INTEGER NOT NULL DEFAULT 0/i);
  });
  ...
  it('inserts nothing into it', async () => {
    ...
    // An absent row and a row of zeroes mean the same thing, and the repository
    // answers both with the defaults.
    expect(statementsFrom(spy).join(' ')).not.toMatch(/INSERT INTO notification_preferences/i);
    expect(spy.runAsync).not.toHaveBeenCalled();
  });

  it('carries no time of day or day of week', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });
...
describe('runMigrations upgrading a version 4 database', () => {
  it('resolves without error', async () => {
  ...
  it('adds the reminder table and the sync state table', async () => {
    const spy = createDatabaseSpy({ user_version: 4 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).toMatch(/CREATE TABLE notification_preferences/i);
    expect(sql).not.toMatch(/CREATE TABLE cycle_settings/i);
    expect(sql).not.toMatch(/CREATE TABLE period_records/i);
    expect(sql).not.toMatch(/CREATE TABLE pregnancy_profile/i);
    expect(sql).not.toMatch(/CREATE TABLE avatar_config/i);
  });

  it('leaves everything already stored alone', async () => {
    const spy = createDatabaseSpy({ user_version: 4 });
    ...
    expect(sql).not.toMatch(
      /UPDATE (cycle_settings|period_records|pregnancy_profile|avatar_config)/i
    );
    expectOnlyAdditiveAlters(sql);
  });

  it('bumps the version to 5 without replaying the earlier steps', async () => {
    const spy = createDatabaseSpy({ user_version: 4 });
    ...
    for (const earlier of [1, 2, 3, 4]) {
      expect(statements).not.toContain(`PRAGMA user_version = ${earlier}`);
    }
  });
  ...
  it('issues no statements outside those transactions', async () => {
    const spy = createDatabaseSpy({ user_version: 4 }, { runTransactionTask: false });
...
describe('runMigrations when the version 5 step fails', () => {
  it('rejects with the underlying error', async () => {
    const spy = createDatabaseSpy({ user_version: 4 }, { execError: new Error('disk is full') });

    await expect(runMigrations(spy.db)).rejects.toThrow('disk is full');
  });

  it('never bumps user_version to 5', async () => {
    const spy = createDatabaseSpy({ user_version: 4 }, { execError: new Error('disk is full') });

    await expect(runMigrations(spy.db)).rejects.toThrow();

    expect(statementsFrom(spy)).not.toContain('PRAGMA user_version = 5');
  });
});

describe('runMigrations applying version 6 to an empty database', () => {
  it('creates the sync state table', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).toMatch(/CREATE TABLE sync_state/i);
  });

  it('keys it on the account rather than pinning it to one row', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).toMatch(/uid TEXT PRIMARY KEY NOT NULL/i);
    expect(sql).not.toMatch(/sync_state \( id INTEGER PRIMARY KEY CHECK \(id = 1\)/i);
  });

  it('gives it the five columns the base is made of', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });
  ...
  it('refuses a negative revision at the database as well', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).toMatch(/CHECK \(revision >= 0\)/i);
  });

  it('refuses blank text where a value is required', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).toMatch(/CHECK \(length\(uid\) > 0\)/i);
    expect(sql).toMatch(/CHECK \(length\(content_hash\) > 0\)/i);
    expect(sql).toMatch(/CHECK \(length\(base_payload\) > 0\)/i);
    expect(sql).toMatch(/CHECK \(length\(updated_at\) > 0\)/i);
  });

  it('writes no row: no base means no sync yet', async () => {
    const spy = createDatabaseSpy({ user_version: 0 });

    await runMigrations(spy.db);
    const sql = statementsFrom(spy).join(' ');

    expect(sql).not.toMatch(/INSERT INTO sync_state/i);
    expect(spy.runAsync).not.toHaveBeenCalled();
  });
});

describe('runMigrations upgrading a version 5 database', () => {
  it('resolves without error', async () => {
  ...
  it('leaves every table that already held data alone', async () => {
    ...
    // No drop, no rebuild, no rewrite: an upgrade cannot cost a period record.
    for (const table of [
      'cycle_settings',
      'period_records',
      'pregnancy_profile',
    ...
    // `notification_preferences` is altered from here on, because version 8
    // adds a column to it. Adding one is not rewriting a row, and this test is
    // about not costing somebody a period record.
    expectOnlyAdditiveAlters(sql);
  });

  it('writes no data at all', async () => {
    const spy = createDatabaseSpy({ user_version: 5 });

    await runMigrations(spy.db);

    expect(statementsFrom(spy).join(' ')).not.toMatch(/INSERT INTO/i);
    expect(spy.runAsync).not.toHaveBeenCalled();
  });
  ...
  it('issues no statements outside that transaction', async () => {
    const spy = createDatabaseSpy({ user_version: 5 }, { runTransactionTask: false });
  ...
  it('runs nothing again on a second pass', async () => {
    const upgraded = createDatabaseSpy({ user_version: 8 });
...
describe('runMigrations when the version 6 step fails', () => {
  it('rejects with the underlying error', async () => {
    const spy = createDatabaseSpy({ user_version: 5 }, { execError: new Error('disk is full') });

    await expect(runMigrations(spy.db)).rejects.toThrow('disk is full');
  });

  it('never claims the version it did not reach', async () => {
    const spy = createDatabaseSpy({ user_version: 5 }, { execError: new Error('disk is full') });

    await runMigrations(spy.db).catch(() => undefined);

    // The bump shares the transaction with the DDL, so a failure leaves the
    // database reporting 5.
    expect(statementsFrom(spy)).not.toContain('PRAGMA user_version = 6');
  });
});
